...
**Recall floor.** When the original query returns fewer than `LOW_RESULT_THRESHOLD` (5; raised from 3 on 2026-06-26) strict-template hits, the server calls `gpt-4o-mini` for 1-3 alternate phrasings PLUS up to 6 decomposition slots (P0.2 multi-query, 2026-06-26), scores each path against the catalog, and unions the results into the page.
...
**Tracking is the post-rewrite count.** The `search_noresult` / `search_lowresult` events fire from `useEffect` on the FINAL `totalResults` — when the rewrite recovers the query (total ≥ 5 after the 2026-06-26 raise), no event fires. The admin's failing-query backlog stays focused on "tried everything and still empty." When the rewrite ran but didn't recover, contentId gains a `|rw=1` suffix and a `|paths=N` suffix (P0.2) so admin can split "thin after rescue attempt" from "thin without rescue attempt" and "thin after N paths tried" from "thin after no expansion attempt."
