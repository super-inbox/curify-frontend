      ...
      <div className="flex space-x-2">
        {shareLinks.map((link) => (
          <button
            ...
            className={`p-2 rounded-lg text-white transition-colors duration-200 ${link.color} ${
              link.action === 'copy' && copied ? 'bg-green-600 hover:bg-green-700' : ''
            }`}
            title={link.action === 'copy' && copied ? 'Copied!' : link.name}
          >
            {link.icon}
          </button>
        ))}
