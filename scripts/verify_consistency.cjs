...
namespaces.forEach(ns => {
  ...
  console.log(`\n==================================================`);
  console.log(`Reference (en/${ns}): ${enKeys.length} keys`);
  console.log(`==================================================`);

  locales.forEach(locale => {
    console.log(`\n--------------------------------------------------`);
    console.log(`Analyzing ${locale}/${ns}...`);

    const filePath = path.join(messagesDir, locale, ns);
    if (!fs.existsSync(filePath)) {
      console.log(`❌ Missing entire file: ${locale}/${ns}`);
      return;
    }
      ...
      for (const key in enObj) {
        if (targetObj && targetObj[key]) {
          ...
          if (typeof enObj[key] === 'string') {
            ...
            if (locale === 'es') {
              if (typeof targetObj[key] === 'string' && (targetObj[key].includes('Wird') || targetObj[key].includes('Bestätigung'))) {
    ...
    if (missingKeys.length === 0 && extraKeys.length === 0 && typeMismatches.length === 0) {
      console.log(`✅ Structure matches en/${ns}`);
    } else {
