// Client-safe topic taxonomy. Imports NO nano data (no nano_inspiration.json /
// nano_templates.json) — only taxonomy.json + the topics i18n keys, which are
// small. Client components must import topic helpers from HERE. The data-derived
// registry (topic → templates, built from the ~4MB nano JSON) lives in
// topicRegistry.ts, which is `server-only`. See lib/nano_pure.ts for the same
// pattern and the Fast Data Transfer rationale.
...
export const LOCALIZED_TOPIC_IDS: Set<string> = new Set(
...
// Slim topic shape ({id, isEnabled}) the client TopicNavRow renders. Computed
// server-side via getTopicNavList() (server-only topicRegistry) and passed down
// as a prop so client components never import the data-derived registry.
export type TopicNavItem = { id: string; isEnabled: boolean };

export function normalizeTopicValues(value: unknown): string[] {
...
export const TIER1_TAG_CHILDREN: Record<string, string[]> = taxonomy.tier3;
export const EXPLICIT_CHILD_TOPICS: Record<string, string[]> = taxonomy.tier2;
...
export const EXPLICIT_PARENT_TOPIC = new Map<string, string>();
